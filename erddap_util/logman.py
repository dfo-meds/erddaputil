...
class ErddapLogManager(ErddapUtil):
    ...
    def _init(self):
        self.erddap_directory = self.config.as_path(("erddap_util", "big_parent_dir"))
        ...
        self.log_retention_days = self.config.as_str(("erddap_util", "log_retention_days"), default="31")
    ...
    def _run(self, *args, **kwargs):
        ...
        for file in os.scandir(self.log_path):
            if file.name.startswith("logArchivedAt") and file.stat().m_time < cutoff:
