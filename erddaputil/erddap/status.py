...
class ErddapStatusScraper(BaseThread):
    ...
    def _run(self, *args, **kwargs):
        ...
        if not self.base_url:
            return False
        self._log.out(f"Downloading status.html and parsing for statistics")
        ...
        self._log.info("Checking startup time...")
        ...
        if last_startup != self._remember['startup_time']:
            self._log.info("Resetting metrics since startup_time has changed")
            self._remember = {
        ...
        self._log.info("Extracting metrics")
        ...
        if esp.has_info('major_load_time_series'):
            self._log.info(f"Parsing time series")
            for ml_info in esp.info['major_load_time_series']:
                self._log.info(ml_info[0])
        ...
        if esp.has_info('languages_since_startup'):
            self._log.info(f"Parsing languages list")
        ...
        self._log.info("Parsing SgtMap stats")
        ...
        self._log.info(f"Parsing map sizes for String2")
        ...
        if esp.has_info('threads'):
            self._log.info("Parsing thread list")
            by_state = {}
        ...
        self._log.out(f"Metric parsing complete")
